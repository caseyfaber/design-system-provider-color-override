...
export class TreeItem extends FASTElement {
  ...
  public childItemLength(): number {
    const treeChildren: HTMLElement[] = this.childItems.filter((item: HTMLElement) => {
  ...
  private collapseOrFocusParent(): void {
    if (this.expanded) {
      ...
      const parentTreeItemNode: Element | null | undefined = this.parentElement!.closest("[role='treeitem']");
